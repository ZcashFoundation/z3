...
That's the `/ready` probe doing its job, not a fault. `/ready` requires the configured peer minimum, a sync status close to tip, a present chain tip, a local tip age within `ready_max_tip_age`, and estimated lag within `ZEBRA_HEALTH__READY_MAX_BLOCKS_BEHIND`. Mainnet uses Zebra's default lag limit of `2`; Testnet uses `4` to tolerate the observed `lag=3` transition during a canonically current 225-second gap. Estimated lag comes from the best tip's header time, while tip age starts when the local height increases, so either independent check can return 503. The response body identifies `insufficient peers`, `syncing`, `no tip`, `tip_age=...`, or `lag=... blocks`. A fresh start from a cold chain, or a restart on a cache that's a few minutes behind, remains `unhealthy` until every readiness check passes.
